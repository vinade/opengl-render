...
def create_model(inputShape):
    ...
    model.add(BatchNormalization())
    model.add(Activation("relu"))
    model.add(Conv2D(FILTERS, KERNEL_SIZE_5, strides=(2, 2), padding="same"))
    model.add(BatchNormalization())
    model.add(Activation("relu"))
    model.add(Conv2D(3, KERNEL_SIZE_5, strides=(2, 2), padding="same"))
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Flatten())
    model.add(Dense(900))
    # model.add(Dense(20*15))
    model.add(Reshape((15, 20, 3)))
...
# training_schedule = []
training_schedule = [10, 10, 5, 4, 3, 2]
