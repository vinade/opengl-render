...
def create_model(inputShape):
    ...
    img1 = Input(inputShape)
    cnn_img1 = Conv2D(FILTERS, (5, 5), padding="same")(img1)
    cnn_img1 = Activation("relu")(cnn_img1)
    cnn_img1 = Conv2D(HALF_FILTERS, (5, 5), padding="same")(cnn_img1)
    cnn_img1 = Activation("relu")(cnn_img1)
    cnn_img1_model = Model(inputs=img1, outputs=cnn_img1)

    bi_model_feature = Conv2D(FILTERS, (5, 5), strides=(
        2, 2), padding="same")(cnn_img1_model.output)
    bi_model_feature = Activation("relu")(bi_model_feature)
    bi_model_feature = Conv2D(FILTERS, (5, 5), strides=(
        2, 2), padding="same")(bi_model_feature)
    bi_model_feature = Activation("relu")(bi_model_feature)
    bi_model_feature = Conv2D(FILTERS, (5, 5), strides=(
        2, 2), padding="same")(bi_model_feature)
    bi_model_feature = Activation("relu")(bi_model_feature)

    bi_model_feature = Conv2D(DOUBLE_FILTERS, (5, 5),
                              padding="same")(bi_model_feature)
    bi_model_feature = Activation("sigmoid")(bi_model_feature)
    bi_model_feature = Conv2D(DOUBLE_FILTERS, (5, 5),
                              padding="same")(bi_model_feature)
    bi_model_feature = Activation("relu")(bi_model_feature)

    bi_model_feature = UpSampling2D(interpolation='bilinear')(bi_model_feature)
    bi_model_feature = Conv2D(
        FILTERS, (5, 5), padding="same")(bi_model_feature)
    bi_model_feature = Activation("relu")(bi_model_feature)
    bi_model_feature = UpSampling2D(interpolation='bilinear')(bi_model_feature)
    bi_model_feature = Conv2D(
        FILTERS, (5, 5), padding="same")(bi_model_feature)
    ...
    bi_model_feature = Conv2D(
        FILTERS, (5, 5), padding="same")(bi_model_feature)
    bi_model_feature = Activation("relu")(bi_model_feature)
    m_bi_model_feature = Model(
        inputs=cnn_img1_model.input, outputs=bi_model_feature)

    bi_model_texture = Conv2D(2, (5, 5), padding="same")(cnn_img1_model.output)
    bi_model_texture = Activation("relu")(bi_model_texture)
    bi_model_texture = Conv2D(HALF_FILTERS, (5, 5),
                              padding="same")(bi_model_texture)
    bi_model_texture = Activation("relu")(bi_model_texture)
    bi_model_texture = Conv2D(
        FILTERS, (5, 5), padding="same")(bi_model_texture)
    bi_model_texture = Activation("relu")(bi_model_texture)
    bi_model_texture = Conv2D(DOUBLE_FILTERS, (5, 5),
                              padding="same")(bi_model_texture)
    bi_model_texture = Activation("relu")(bi_model_texture)
    bi_model_texture = Conv2D(
        FILTERS, (5, 5), padding="same")(bi_model_texture)
    ...
    bi_model_texture = Conv2D(HALF_FILTERS, (5, 5),
                              padding="same")(bi_model_texture)
    bi_model_texture = Activation("relu")(bi_model_texture)
    bi_model_texture = Conv2D(1, (3, 3), padding="same")(bi_model_texture)
    bi_model_texture = Activation("relu")(bi_model_texture)
    m_bi_model_texture = Model(
        inputs=cnn_img1_model.input, outputs=bi_model_texture)

    combined = concatenate(
        [m_bi_model_feature.output, m_bi_model_texture.output])
    output_model = Conv2D(HALF_FILTERS, (5, 5), padding="same")(combined)
    output_model = Activation("relu")(output_model)
    output_model = Conv2D(FILTERS, (5, 5), padding="same")(combined)
    output_model = Activation("relu")(output_model)
    output_model = Conv2D(1, (3, 3), padding="same")(output_model)

    return Model(inputs=cnn_img1_model.input, outputs=output_model)
...
# training_schedule = []
training_schedule = [50, 40, 30, 20, 10, 10, 5,
                     5, 5, 5, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2]
