...
def create_model(inputShape):
    ...
    model.add(DepthwiseConv2D(KERNEL_SIZE_3, strides=(2, 2), padding="same"))
    ...
    model.add(DepthwiseConv2D(KERNEL_SIZE_5, strides=(2, 2), padding="same"))
    model.add(Conv2D(OCT_FILTERS, KERNEL_SIZE_1, padding="same"))
    model.add(Activation("relu"))

    model.add(DepthwiseConv2D(KERNEL_SIZE_5, padding="same"))
