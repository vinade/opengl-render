...
class DepthModel:
    ...
    def create_pre_filter(self, block_input):
        ...
        layers.append(DepthwiseConv2D(
            Kernel.K3,
            padding="same"))
        layers.append(Conv2D(FILTERS, Kernel.K1))
    ...
    def create_post_filter(self, block_input):
        ...
        layers.append(DepthwiseConv2D(
            Kernel.K3,
            padding="same"))
        layers.append(Conv2D(1, Kernel.K1, padding="same"))
        # layers.append(Activation("relu"))
    ...
    def create_block_filter(self, block_input, filters, blocks, output_size=None):
        output_size = output_size or filters
        layers = []
        ...
        layers.append(DepthwiseConv2D(
            Kernel.K3,
            padding="same"))
        layers.append(Conv2D(output_size, Kernel.K1, padding="same"))
        layers.append(Activation("relu"))
    ...
    def create_middle_filter2(self, block_input):
        ...
        layers.append(DepthwiseConv2D(
            Kernel.K3,
            padding="same"))
        layers.append(Conv2D(HALF_FILTERS, Kernel.K1))
        layers.append(Activation("relu"))
    ...
    def create_model(self, input_shape=None, model_name=None):
        ...
        middle_filter1_model = self.create_middle_filter1(
            pre_filter_model)
        middle_filter2_model = self.create_block_filter(
            pre_filter_model, HALF_FILTERS, 3)

        middle_filter = concatenate(
            [middle_filter1_model, middle_filter2_model])

        post_filter_model = self.create_block_filter(
            middle_filter, FILTERS, 3, 1)
