...
def create_model(inputShape):
    ...
    model.add(DepthwiseConv2D(KERNEL_SIZE_3, strides=(
        2, 2), padding="same", depth_multiplier=10))
    ...
    model.add(DepthwiseConv2D(KERNEL_SIZE_5, strides=(
        2, 2), padding="same", depth_multiplier=10))
    model.add(Conv2D(OCT_FILTERS, KERNEL_SIZE_1, padding="same"))
    model.add(Activation("relu"))

    model.add(DepthwiseConv2D(KERNEL_SIZE_5,
                              padding="same", depth_multiplier=10))
    model.add(Conv2D(TETRA_FILTERS, KERNEL_SIZE_1, padding="same"))
    ...
    model.add(DepthwiseConv2D(KERNEL_SIZE_3, padding="same"))
    model.add(Conv2D(FILTERS, KERNEL_SIZE_1, padding="same"))
    model.add(Activation("relu"))

    model.add(UpSampling2D(interpolation='bilinear'))  # ////////
    model.add(DepthwiseConv2D(KERNEL_SIZE_3, padding="same"))
    model.add(Conv2D(DOUBLE_FILTERS, KERNEL_SIZE_1, padding="same"))
    model.add(Activation("relu"))
