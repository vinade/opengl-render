...
from keras.layers import BatchNormalization, Reshape, Flatten
...
data_loader = DepthDataLoader(IMAGES_N, (139, 99))
...
def create_model(inputShape):
    ...
    model.add(Conv2D(FILTERS, KERNEL_SIZE_7, strides=(2, 2)))
    model.add(Activation("relu"))
    # model.add(Conv2D(DOUBLE_FILTERS, KERNEL_SIZE_7,
    #                  strides=(2, 2)))
    # model.add(Activation("relu"))
    model.add(Conv2D(TETRA_FILTERS, KERNEL_SIZE_7))
    model.add(Activation("relu"))

    # model.add(Conv2D(OCT_FILTERS, KERNEL_SIZE_5))
    # model.add(BatchNormalization())
    # model.add(Activation("relu"))
    model.add(Conv2D(HEX_FILTERS, KERNEL_SIZE_5))
    model.add(Activation("relu"))
    model.add(Conv2D(OCT_FILTERS, KERNEL_SIZE_3))
    model.add(Activation("relu"))

    model.add(Conv2D(TETRA_FILTERS, KERNEL_SIZE_3))
    model.add(Activation("relu"))
    model.add(Conv2D(DOUBLE_FILTERS, KERNEL_SIZE_3))
    model.add(Activation("relu"))
